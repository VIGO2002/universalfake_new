...
class EffortDetector(nn.Module):
    ...
    def build_backbone(self, config):
        ...
        # ViT-L/14 224*224
        # 修改后（注意路径要用引号括起来）
        clip_model = CLIPModel.from_pretrained("/root/autodl-tmp/pretrained_models/clip-vit-large-patch14")  # the path of this folder in your disk (download from the above link)
    ...
    def get_losses(self, data_dict: dict, pred_dict: dict) -> dict:
        label = data_dict['label']
        energy = pred_dict['cls'].squeeze()

        # --- 1. EBM 核心损失 (Decision Term) ---
        m_real = -10.0
        m_fake = 10.0
        ...
        loss_ebm = loss_real + loss_fake

        # --- 2. Effort 正则化损失 (Structure Term) ---
        loss_reg = torch.tensor(0.0).to(energy.device)
        if self.training:
            lambda_reg = 0.5  # 权重可以沿用 Effort 原文的设定
            reg_term = 0.0
            num_reg = 0
            # 遍历所有模块寻找 SVD 层
            for module in self.backbone.modules():
                # 注意：需要确保 SVDResidualLinear 类在当前作用域可见，或者引用正确
                if hasattr(module, 'compute_orthogonal_loss') and hasattr(module, 'compute_keepsv_loss'):
                    reg_term += module.compute_orthogonal_loss()
                    reg_term += module.compute_keepsv_loss()
                    num_reg += 1

            if num_reg > 0:
                loss_reg = lambda_reg * reg_term / num_reg

        # --- 3. 总损失 ---
        loss_total = loss_ebm + loss_reg

        loss_dict = {
            'overall': loss_total,
            'real_loss': loss_real,
            'fake_loss': loss_fake,
            'reg_loss': loss_reg  # 记录一下方便观察
        }
        return loss_dict
