...
class Trainer(object):
    ...
    def train_step(self, data_dict):
            # ============================================================
            # [新增模块] 鲁棒流形正则化 (Robust Manifold Regularization)
            # 说明：在训练阶段向输入数据注入微量高斯噪声，防止 EBM 对源域(FF++)过拟合，
            # 从而扩宽“真实样本”的能量低谷，提升对 DFDCP 等噪声数据的泛化性。
            # ============================================================
            if self.config.get('use_noise_injection', False): # 建议在yaml里开启这个开关
                # 获取噪声强度，默认为 0.01 (这是一个经验值，可根据效果微调)
                noise_std = self.config.get('noise_std', 0.01) 
                
                # 生成与输入图像同维度的正态分布噪声
                noise = torch.randn_like(data_dict['image']) * noise_std
                
                # 将噪声注入到输入图像中 (Input Noise Injection)
                # 注意：这里我们使用 += 也就是原地修改，确保后续模型读到的是带噪数据
                data_dict['image'] = data_dict['image'] + noise.to(self.model.device)
            # ============================================================
